...
import { 
  ...
  Heading, Type, AlignLeft, Grid, Layout, CheckSquare, Zap, Settings2,
  Trash, PlusCircle, MinusCircle, HelpCircle, Search, X
} from 'lucide-react';
...
const FIELD_TYPE_CONFIG = {
  ...
  wysiwyg:      { label: 'Rich Text',            icon: '✨',  color: '#a855f7', desc: 'Editor rich text no formulário' },
...
export default function CreateTemplate({ onClose, onSuccess }) {
  ...
  const [blocks, setBlocks] = useState([
    { id: 'b1', type: 'title', data: { text: 'CONTRATO DE PRESTAÇÃO DE SERVIÇOS' } },
    { id: 'b2', type: 'text', data: { content: 'Pelo presente instrumento, eu, {nome_cliente}, aceito os termos acordados a partir da data de {data_inicio}.\n\nFica eleito o foro da comarca local para dirimir quaisquer dúvidas.' } }
  ]);
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      ...
      setBlocks([
        { id: 'b1', type: 'title', data: { text: 'CONTRATO DE PRESTAÇÃO DE SERVIÇOS' } },
        { id: 'b2', type: 'text', data: { content: 'Texto do contrato...' } }
      ]);
  ...
  const getFieldSemanticLabel = (field) => {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
        ...
        {realType === 'number' && (config.min !== undefined || config.max !== undefined) && (
          <span style={{ fontSize: '10px', padding: '2px 6px', borderRadius: '4px', backgroundColor: 'rgba(249,115,22,0.12)', color: '#f97316', border: '1px solid rgba(249,115,22,0.2)' }}>
            🔢 {config.min || 0} - {config.max || '∞'}
          </span>
        )}
  ...
  return (
    <div className={onClose ? "" : "container"} style={onClose ? { padding: '24px 32px' } : {}}>
      ...
      {currentStep === 1 && (
        <div className="card">
          ...
          <div style={{ borderTop: '1px solid var(--bg-card-border)', paddingTop: '24px' }}>
                ...
                fields.map((field, idx) => {
                  ...
                  return (
                    <div key={idx} style={{
                      ...
                      <div onClick={() => handleExpandField(idx)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 14px', cursor: 'pointer', userSelect: 'none' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginLeft: '10px', flexShrink: 0 }}>
                          ...
                          <button onClick={(e) => { e.stopPropagation(); handleRemoveField(idx); }} className="btn btn-danger" style={{ padding: '5px', borderRadius: '6px' }} title="Remover campo">
                            <Trash2 size={13} />
                          </button>
                        </div>
                      </div>
                      ...
                      {isExpanded && efd && (
                        <div style={{ padding: '16px', borderTop: `1px solid ${typeInfo?.color || 'var(--primary)'}25`, backgroundColor: 'rgba(0,0,0,0.15)', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                          ...
                          {eNeedsOptions && (
                            <div className="input-group" style={{ marginBottom: 0 }}>
                              ...
                              <input type="text" className="input-field" placeholder="Ex: Sim, Não, Talvez" value={efd.options} onChange={e => setEditingFieldData({ ...efd, options: e.target.value })} />
                            </div>
                          )}
                          ...
                          <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end', paddingTop: '4px' }}>
                            <button type="button" className="btn btn-secondary" style={{ padding: '8px 16px', fontSize: '13px' }} onClick={() => { setExpandedFieldIdx(null); setEditingFieldData(null); }}>Cancelar</button>
                            <button type="button" className="btn btn-primary" style={{ padding: '8px 20px', fontSize: '13px', backgroundColor: typeInfo?.color, borderColor: typeInfo?.color }} onClick={() => handleUpdateField(idx)}>✓ Atualizar Campo</button>
                          </div>
                        </div>
                      )}
                    </div>
            ...
            {/* Add Field Form */}
            <form onSubmit={handleAddField} style={{ backgroundColor: 'rgba(99,102,241,0.04)', padding: '20px', borderRadius: 'var(--radius-md)', border: '1px solid rgba(99,102,241,0.15)' }}>
              ...
              {currentTypeInfo && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 14px', borderRadius: 'var(--radius-sm)', marginBottom: '16px', backgroundColor: `${currentTypeInfo.color}10`, border: `1px solid ${currentTypeInfo.color}25` }}>
              ...
              {newFieldType === 'number' && (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                  <div className="input-group" style={{ marginBottom: 0 }}>
                    <label className="input-label">Valor Mínimo Aceito (Opcional)</label>
                    <input type="number" className="input-field" placeholder="Ex: 1711" value={newFieldMin} onChange={(e) => setNewFieldMin(e.target.value)} />
                  </div>
                  <div className="input-group" style={{ marginBottom: 0 }}>
                    <label className="input-label">Valor Máximo Aceito (Opcional)</label>
              ...
              {needsOptions && (
                <div className="input-group" style={{ marginBottom: '16px' }}>
                  <label className="input-label">Opções <span style={{ color: 'var(--text-muted)', fontWeight: 'normal' }}>(separadas por vírgula)</span></label>
              ...
              {!isLayoutType && (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '16px', marginBottom: '16px', alignItems: 'end' }}>
                  <div className="input-group" style={{ marginBottom: 0 }}>
                    <label className="input-label">Placeholder (Dica)</label>
                    <input type="text" className="input-field" placeholder="Ex: Razão Social Completa" value={newFieldPlaceholder} onChange={(e) => setNewFieldPlaceholder(e.target.value)} />
                  </div>
              ...
              {!isLayoutType && (
                <div style={{ marginBottom: '16px' }}>
                  <button type="button" onClick={() => setShowAdvanced(!showAdvanced)} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: showAdvanced ? 'rgba(245,158,11,0.1)' : 'rgba(255,255,255,0.02)', border: showAdvanced ? '1px solid rgba(245,158,11,0.3)' : '1px solid var(--bg-card-border)', color: showAdvanced ? '#f59e0b' : 'var(--text-secondary)', padding: '8px 14px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', fontWeight: '600' }}>
                    <Zap size={14} />
                    {showAdvanced ? '▾ Ocultar Lógica Condicional' : '⚡ Adicionar Condicional'}
                  </button>

                  {showAdvanced && (
                    <div style={{ marginTop: '12px', padding: '16px', borderRadius: 'var(--radius-sm)', backgroundColor: 'rgba(245,158,11,0.05)', border: '1px solid rgba(245,158,11,0.2)' }}>
      ...
      {/* ===== STEP 2: BLOCK TEMPLATE BUILDER ===== */}
      {currentStep === 2 && (
        <div className="card">
          <h2 style={{ fontSize: '20px', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <FileText size={20} color="var(--primary)" /> Construtor de Documento por Blocos
          </h2>
          <p style={{ color: 'var(--text-secondary)', marginBottom: '24px', fontSize: '14px' }}>
            Construa seu modelo organizando seções estruturadas. Clique nos campos da barra lateral para injetá-los no bloco ativo.
          </p>
          ...
          <div className="editor-layout" style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '24px' }}>
                    ...
                    {block.type === 'title' && (
                      <div className="input-group" style={{ marginBottom: 0 }}>
                        <input
                          ...
                          onFocus={() => setActiveInput({ blockId: block.id, fieldKey: 'text', selectionStart: e.target?.selectionStart })}
                    ...
                    {block.type === 'text' && (
                      <div className="input-group" style={{ marginBottom: 0 }}>
                        <textarea
                          className="input-field"
                          rows={6}
                          placeholder="Digite o conteúdo do texto. Use chaves {campo} para inserir variáveis do formulário."
                    ...
                    {block.type === 'clause' && (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        <input
                          type="text"
                          className="input-field"
                          placeholder="Título da Cláusula (Ex: Cláusula Primeira - Do Objeto)"
            ...
            <div className="sidebar-panel" style={{ height: 'fit-content', position: 'sticky', top: '20px' }}>
              <h3 style={{ fontSize: '14px', fontWeight: '700', color: 'white', borderBottom: '1px solid var(--bg-card-border)', paddingBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <Zap size={14} color="var(--primary)" /> Variáveis de Formulário
              </h3>
              <p style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '14px' }}>
                Clique em um campo de texto de um bloco acima e depois clique na variável para inseri-la.
              </p>

              {insertableFields.length === 0 ? (
                <p style={{ fontSize: '12px', color: 'var(--text-muted)', textAlign: 'center', padding: '20px 0' }}>
                  Nenhum campo cadastrado no passo anterior.
                </p>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', overflowY: 'auto', maxHeight: '400px' }}>
                  {insertableFields.map((field) => {
                    ...
                    const typeInfo = FIELD_TYPE_CONFIG[realType];
                    return (
                      <div key={field.key} className="variable-badge" onClick={() => insertVariable(field.key)} style={{ cursor: 'pointer', transition: 'all 0.15s ease' }} title="Clique para injetar no bloco ativo">
      ...
      {currentStep === 3 && (
        <div className="card">
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid var(--bg-card-border)', paddingTop: '24px' }}>
            ...
            <button className="btn btn-primary" onClick={handleSaveTemplate} disabled={isLoading} style={{ paddingLeft: '32px', paddingRight: '32px' }}>
